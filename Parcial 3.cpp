...
// ===========================================================================
// FUNCIONES PARA LOS NUEVOS REPORTES SOLICITADOS
// ===========================================================================
...
void mostrarProductosMenorStock() {
    ...
    int n = productosOrdenados.size();
      for (int i = 0; i < n - 1; i++) {
         for (int j = 0; j < n - i - 1; j++) {
            if (productosOrdenados[j].stock > productosOrdenados[j + 1].stock) {
    ...
    for (int i = 0; i < cantidad; i++) {
        cout<<productosOrdenados[i].idProducto<< " | "; 
        cout<<productosOrdenados[i].nombre<<" | "; 
        cout<<productosOrdenados[i].stock<<endl;
    }
    if (productosOrdenados.empty()) {
        cout << "No hay productos disponibles" << endl;
    }
}

// b. Mostrar cantidad de comentarios en una fecha específica
void mostrarComentariosPorFecha() {
    system("cls");
    string fecha;
    cout << "\nIngrese la fecha (formato: aaaa-mm-dd): ";
    cin >> fecha;
    
    // Validar formato básico de fecha
    if (fecha.length() != 10 || fecha[4] != '-' || fecha[7] != '-') {
        cout << "Error: Formato de fecha inválido. Use aaaa-mm-dd" << endl;
        return;
    }
    int contador = 0;
    for (size_t i = 0; i < comentarios.size(); i++) {
    ...
    cout<<"\n=== Comentarios en la fecha "<<fecha<<" ==="<<endl<<endl;
    cout<<"Fecha: "<<fecha<<endl;
    cout<<"Cantidad de comentarios: "<<contador<<endl;
    
}

// c. Mostrar precio máximo y mínimo de los productos
void mostrarPrecioMaximoMinimo() {
    system("cls");
    if (productos.empty()) {
        cout<<"\nNo hay productos disponibles"<<endl;
        return;
    }
    
    double precioMaximo = productos[0].precio;
    double precioMinimo = productos[0].precio;
    Producto productoMaximo = productos[0];
    Producto productoMinimo = productos[0];
    
    for (size_t i = 1; i < productos.size(); i++) {
        if (productos[i].precio > precioMaximo) {
            precioMaximo = productos[i].precio;
            productoMaximo = productos[i];
        }
        if (productos[i].precio < precioMinimo) {
            precioMinimo = productos[i].precio;
            productoMinimo = productos[i];
        }
    }
    
    cout<<"\n=== Precios Maximo y Minimo ==="<<endl<<endl;
    cout<<"Precio Maximo: $" << precioMaximo<<endl;
    cout<<"Producto con precio máximo: "<<productoMaximo.nombre<<"(ID: "<< productoMaximo.idProducto<<")"<<endl<< endl;
   
    cout<<"Precio Minimo: $" << precioMinimo << endl;
    cout<<"Producto con precio mínimo: " << productoMinimo.nombre  << " (ID: " << productoMinimo.idProducto << ")" << endl;
}

void menuReportesAdicionales() {
    int opcion;
...
void menuUsuario(Usuario* usuario) {
    ...
    do { 
        ...
        cout << "6. Reportes adicionales" << endl; 
